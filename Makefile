...
.PHONY: test
test: ## Run all Go tests (root module + difftest submodule)
	go test ./...
	cd difftest && go test ./...
...
.PHONY: vet
vet: ## go vet the root module and difftest submodule
	go vet ./...
	cd difftest && go vet ./...
...
.PHONY: tidy
tidy: ## go mod tidy the root module and difftest submodule
	go mod tidy
