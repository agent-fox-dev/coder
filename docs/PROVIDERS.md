...
| `openai-completions` | unversioned (`/v1`) | `POST https://api.openai.com/v1/chat/completions` | none — hand-authored fixtures | — | 2026-09-06 | compat profile resolved from the host for every REQ-PROV-12 flag; `cache_control` over Chat Completions (OpenRouter `anthropic/*`) and the 1h TTL are wired. Three flags resolve but nothing branches on them: `ThinkingTokenBudgetField`, `AllowsUserAfterToolResult`, `ThinkingFormat` (`reasoning_content` is echoed unconditionally instead) — see `docs/GAPS.md` |
