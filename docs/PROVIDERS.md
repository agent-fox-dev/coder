...
|---|---|---|---|---|---|---|
| `anthropic-messages` | `anthropic-version: 2023-06-01` (direct) / `anthropic_version: vertex-2023-10-16` (Vertex) | `POST https://api.anthropic.com/v1/messages` — on Vertex, `POST https://{location-}aiplatform.googleapis.com/v1/projects/{project}/locations/{location}/publishers/anthropic/models/{id}:streamRawPredict` | none — hand-authored fixtures | — | 2026-09-09 | full. Both deployments ship and switching is a config change (NFR-COMPAT-05, ruling L-12); on Vertex the version moves into the body and the model becomes a URL segment. Thinking is sent in the generation the catalog row speaks: an effort token becomes `thinking: {type: adaptive}` + `output_config.effort`, an integer becomes `budget_tokens` (never below 1024, never at or above `max_tokens`); `off` sends `disabled` only where the row maps it. Rows with `compat.supports_sampling: false` drop `temperature`/`top_p`. Server-side compaction (REQ-PROV-07) is opt-in per OQ-4 and sends both the beta header and `context_management`. Not implemented: the tool-search server tool (`defer_loading` is therefore never sent — ruling L-10), `top_k`, forced `tool_choice` |
...
| `openai-responses` | unversioned (`/v1`) | `POST https://api.openai.com/v1/responses` | none — hand-authored fixtures | — | 2026-09-09 | full. REQ-CACHE-10 uses the prose arm (ruling L-10): the PRD's `additional_tools` is not a Responses API parameter. `include: ["reasoning.encrypted_content"]` is sent on every request the profile allows it on, independent of the thinking level; a reasoning item is replayed only with its encrypted content under `store: false`, and always with `summary: []`; temperature/top_p are gated on `supports_sampling_params` |
...
| L-10 | `openai-responses` | **`additional_tools` does not exist; a deferred tool is declared in prose.** The PRD's REQ-CACHE-10 arm for this wire named a request parameter the Responses API does not have — a request carrying it is rejected or has the field dropped, leaving a tool the model can neither see nor call. The adapter uses the same prose arm as `openai-completions` (L-8): the tool is withheld from `tools`, which stays byte-identical, and re-declared in a system message item after the tool-result run that introduced it. Same weakness as L-8, same reason. Reversal requires the vendor to ship such a parameter, not a flag. |
...
| L-12 | `anthropic-messages` | **A flag variable MAY select the Vertex deployment; ambient GCP environment may not.** `CLAUDE_CODE_USE_VERTEX` and `ANTHROPIC_VERTEX_PROJECT_ID` are set by nothing except an operator choosing Claude on Vertex, so they select it — unlike L-7's `GOOGLE_CLOUD_PROJECT`, which is set on every GCE and Cloud Run box. That variable keeps L-7's treatment here: it may *supply* a project once something else has selected the deployment, never *select* it. **Amended 2026-09-10** (`docs/errata/01_vertex_deployment_selection.md`): the signals are RANKED, not OR-ed, because these variables are sticky — `Options.VertexProject`, a truthy flag and a Vertex host select outright; the flag read as false is an explicit off that vetoes the rest; `ANTHROPIC_VERTEX_PROJECT_ID` alone selects only when no Anthropic-direct credential is set. A leftover project variable used to route an API-key deployment to Google and die on a 401 naming none of it. A selected deployment with no resolvable project is an error naming the project, not the direct path sent to a Vertex host. **An `x-api-key` credential is dropped on this path**: an Anthropic key is not a Vertex credential, and forwarding the one a machine still carries from its old deployment would hand a first-party secret to a third party. |
