...
> **0.4.2** closes the items 0.4.1 recorded as deferred: deferred requests as OQ-11 option (b),
> the Vertex AI path, Gemini `CachedContent`, the schema cache and deferred tool loading on all
> five wires, the three unconsumed compat flags, the re-arming drain timer, MCP collision at
> connect, the SSE fuzz target, golden provenance, and the REQ-SKILL items. NFR-PERF-08 is
> restored from design guidance; NFR-PERF-02 is re-grounded on the fact that the handshake it
> budgets no longer exists. `docs/GAPS.md` carries the row-by-row record.
>
...
- **NFR-PERF-08 (RESTORED in 0.4.2; the numeric half remains design guidance):** Google `CachedContent` creation is a network call and runs on a background goroutine before the first model call of the session when the context cache is enabled. **The agent loop must not block waiting for it — it falls back to uncached operation if creation has not completed.** That non-blocking half is the testable structural claim this requirement was demoted for lacking, and it now ships: it is pinned by `TestTheAgentLoopNeverWaitsForCachedContentCreation`, whose creation round trip never returns and which therefore DEADLOCKS against a synchronous implementation rather than measuring it. The creation timing itself has no offline benchmark and stays design guidance per NFR-PERF-09.
- **NFR-PERF-09 (performance budgets need an acceptance mechanism):** Every numeric budget in this section is either attached to a named `go test -bench` target with a CI threshold, or it is demoted to design guidance and labelled as such. A number with no benchmark behind it cannot fail, and therefore does not constrain anything. Prioritization note: in the closest shipped comparable — a zero-dependency Go agent SDK of comparable scope — the entire test budget went to differential, golden, parity and race testing and **no Go benchmarks were written at all**, which is evidence that correctness-under-concurrency and wire fidelity are where the defects actually live. If the benchmarks are not going to be written, the honest move is to delete the numbers rather than ship unenforceable ones.
  ...
  **Status.** Four budgets now carry a benchmark and a threshold test that fails the build: NFR-PERF-01 (`BenchmarkLoopTurnOverhead`), NFR-PERF-06 (`BenchmarkCacheHit` against `BenchmarkDirectResponse`), NFR-PERF-07 (`BenchmarkCacheControlStamping`) and NFR-PERF-03 (asserted as a COUNT — a steady-state request must serialize zero schemas — rather than as a duration, so it cannot flake). NFR-PERF-04, NFR-PERF-05 and now NFR-PERF-08's non-blocking clause are structural rather than numeric and are pinned by tests that deadlock rather than by thresholds. NFR-PERF-02 stays demoted: it budgets a handshake the 2026-07-28 revision removed (REQ-MCP-CLIENT-03), so there is no longer a thing to time.

  Two findings came out of writing them. First, REQ-CACHE-06's schema cache was implemented, unit-tested and attached to no provider, so every request re-serialized every tool schema — ~0.9 ms of a ~1.4 ms request build at 128 tools. (It was then attached to Anthropic only, and the other four wires kept re-serializing until 0.4.2; the COUNT assertion now runs per adapter, which is what would have caught the second omission.) Second, NFR-PERF-01's "per turn" is under-specified: the REQ-GO-15 estimate and the REQ-PROV-11 repair pass are both O(history), so a turn 500 messages deep costs ~1.7x a first turn. Both remain well inside 1 ms; the depth term is reported by a benchmark and deliberately not given a threshold the requirement does not state.
